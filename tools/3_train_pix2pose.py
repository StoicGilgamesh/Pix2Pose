...
for X_src,X_tgt,disc_tgt,prob_gt in iter_:
    ...
    if(batch_counter>n_batch_per_epoch):
        ...
        npyfn = weight_dir+"/val_img/"+weight_prefix+"_{:02d}".format(epoch)
        ...
        # plt.close()
        for i in range(10):
            np.save(npyfn+"_"+str(i)+"_X_src.npy", (X_src[i]+1)/2)
            np.save(npyfn+"_"+str(i)+"_X_tgt.npy", (X_tgt[i]+1)/2)
            np.save(npyfn+"_"+str(i)+"_gen_images.npy", (gen_images[i]+1)/2)
