...
def augment_inplane_gen(xyz_id,img,img_r,depth_rend,mask,isYCB=False,step=10):
    ...
    for rot in np.arange(step,360,step):
        ...
        check_array0 = img_npy[:,:,0]
        if np.all(check_array0 == 128):
            print("augment_inplane_gen ################### NULL !!!!!!!!!!!!!!!!!!!! check_array0:2")
            break
        check_array1 = img_npy[:,:,1]
        if np.all(check_array1 == 128):
            print("augment_inplane_gen################### NULL !!!!!!!!!!!!!!!!!!!! check_array0:2")
            break
        check_array2 = img_npy[:,:,2]
        if np.all(check_array2 == 128):
            print("augment_inplane_gen################### NULL !!!!!!!!!!!!!!!!!!!! check_array0:2")
            break
        check_array3 = img_npy[:,:,3]
        if np.all(check_array3 == 0):
            print("augment_inplane_gen################### NULL !!!!!!!!!!!!!!!!!!!! check_array3:5")
            break
        check_array4 = img_npy[:,:,4]
        if np.all(check_array4 == 0):
            print("augment_inplane_gen################### NULL !!!!!!!!!!!!!!!!!!!! check_array3:5")
            break
        check_array5 = img_npy[:,:,5]
        if np.all(check_array5 == 0):
            print("augment_inplane_gen################### NULL !!!!!!!!!!!!!!!!!!!! check_array3:5")
            break
...
for m_id,model_id in enumerate(model_ids):
        ...
        # ply_fn ="/home/stoic_gilgamesh/PhD/RoadmapWithStefan/Canister_px2ps_data/container_nt_cntrd.ply"
        # ply_fn ="/home/stoic_gilgamesh/PhD/RoadmapWithStefan/Pix2Pose/dataset/bottlearch/models/obj_000001_1000Scaled.ply"
        obj_model = Model3D()
        # obj_model.load(ply_fn, scale=0.001)
        obj_model.load(ply_fn)
        # obj_model.vertices[:][:,2] = obj_model.vertices[:][:,2] + 0.05 ######## ONLY FOR CANISTER MODEL!!!!!
        keys = m_info.keys()
